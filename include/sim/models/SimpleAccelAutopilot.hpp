...
public:
     ...
     *
     * @param state        Current missile state (currently unused).
     ...
     */
    [[nodiscard]] std::vector<double> computeActuatorCommand(
        const sim::State& state,
        const std::vector<double>& desiredAccel) override;

private:
    double gain_;    ///< Scalar gain on guidance acceleration.
